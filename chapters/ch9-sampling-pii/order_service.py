...
#   • New /ask endpoint simulating a GenAI call — sets llm.prompt,
#     llm.completion, and user.query span attributes containing PII.
#     The Collector's transform processor scrubs these before export.
#   • All existing error patterns (Pattern 1, 2, 3) preserved from ch7/ch8.
#   • No application-side sampling — sampling is done in the Collector.
#
# Demonstrates:
...
#   GenAI sim — PII in span attributes (/ask endpoint)
import asyncio
import random
import time
...
async def simulate_llm_generation(prompt: str) -> dict:
    ...
    This intentionally includes PII in both the prompt and completion
    to demonstrate the Collector's transform processor scrubbing.
    """
    await asyncio.sleep(0.5 + random.uniform(0, 0.5))  # 500–1000 ms
    if random.random() < 0.1:
        raise ConnectionError("LLM API timeout — no response after 5000 ms")

    # Simulated completion that echoes back PII (realistic worst-case)
    completion = (
...
#
# Simulates an LLM-powered endpoint that receives a user question,
# sends it to a (simulated) LLM, and returns a completion.
#
# The span intentionally carries PII in these attributes:
#   • llm.prompt      — full user prompt (may contain names, SSNs, emails)
#   • llm.completion   — full LLM response (may echo PII back)
#   • user.query       — the raw user question
#   • llm.model        — model identifier (safe, kept after scrubbing)
#   • llm.tokens_used  — token count (safe, kept after scrubbing)
#
# The Collector's transform processor scrubs llm.prompt, llm.completion,
# and PII patterns in user.query BEFORE the trace reaches Jaeger.
# ──────────────────────────────────────────────
@app.post("/ask")
async def ask_question(body: dict):
    """GenAI-style endpoint: user asks a question, LLM responds."""
    # Simulate a user question containing PII
    user_query = body.get(
        "question",
    ...
    with tracer.start_as_current_span("llm_generation") as span:
        # Set span attributes that contain PII — the Collector will scrub these.
